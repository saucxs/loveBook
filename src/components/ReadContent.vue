  ...
  export default {
    ...
    created() {
      ...
      if (!this.skinBgList.includes(this.skinColor)) {
        this.$store.commit('SET_SKIN_COLOR','skin-default');
      }
    },
    methods: {
      ...mapActions([
      ...
      switchMode: function () {
        ...
        this.$store.commit('SET_NIGHT_MODE',!this.nightMode);
      },
      showOpt() {
      ...
      changeFontSize(isAdd) {
        ...
        isAdd ? size++ : size--
        this.$store.commit('SET_FONT_SIZE',size);
      },
      changeBkColor(skin) {
        this.$store.commit('SET_NIGHT_MODE',false);
        this.$store.commit('SET_SKIN_COLOR',skin);
      },
      addToShelf: function() {
        if (this.isAdded) {
          return;
